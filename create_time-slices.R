...
# include wares: 47, 50, 75, 79, 81, 82, 90, 91 ? --> No chronological information

datawd = "C:/Users/u0112360/Documents/____/Sagalassos/__PhD/Articles/Time_Averaging/time-averaged-networks-master/time-averaged-networks-master/Data/ICRATES/"
data_catalogue = read.csv(paste0(datawd, "ICRATES_CATALOGUE.csv"), header = T, sep = ",")
data_form = read.csv(paste0(datawd, "ICRATES_STANDARD_FORM.csv"), header = T, sep = ",")
...
# setwd("C:/Users/u0112360/Documents/____/Sagalassos/__PhD/Articles/__MI-Article/results/ICRATES_slices")
...
setwd("C:/Users/u0112360/Documents/____/Sagalassos/__PhD/Articles/__MI-Article/results/ICRATES_slices")
# Hellenistic (-150, -50]
df_slice_1 = as.data.frame.matrix(xtabs(df_agg_fabric_loc[,7] ~ Location_ID + Fabric_ID, data = df_agg_fabric_loc))
df_slice_2 = as.data.frame.matrix(xtabs(df_agg_fabric_loc[,8] ~ Location_ID + Fabric_ID, data = df_agg_fabric_loc))
df_slice_HELLENISTIC = df_slice_1 + df_slice_2
# write.csv(df_slice_HELLENISTIC, "slice_HELLENISTIC.csv", row.names = TRUE)
# Roman (-50, 50]
df_slice_1 = as.data.frame.matrix(xtabs(df_agg_fabric_loc[,9] ~ Location_ID + Fabric_ID, data = df_agg_fabric_loc))
df_slice_2 = as.data.frame.matrix(xtabs(df_agg_fabric_loc[,10] ~ Location_ID + Fabric_ID, data = df_agg_fabric_loc))
df_slice_ROMAN = df_slice_1 + df_slice_2
# write.csv(df_slice_ROMAN, "slice_ROMAN.csv", row.names = TRUE)
