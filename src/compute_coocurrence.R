...
# read aggregated slices
slice_Hellenistic <- read.csv(here("data", "slice_aggr_HELLENISTIC.csv"))
colnames(slice_Hellenistic)[1] <- "SiteCode"
slice_Roman <- read.csv(here("data", "slice_aggr_ROMAN.csv"))
...
for (i in 1:(dim(slice_Hellenistic_PA)[1] - 1)) {
  for (j in (i + 1):dim(slice_Hellenistic_PA)[1]) {
    newline <- data.frame(
      "Var1" = slice_Hellenistic_PA$SiteCode[i],
      "Var2" = slice_Hellenistic_PA$SiteCode[j],
      "CO_Hellenistic" = (sum(
        slice_Hellenistic_PA[i, 2:6] == slice_Hellenistic_PA[j, 2:6]
      )) / 5
    )
...
for (i in 1:(dim(slice_Roman_PA)[1] - 1)) {
  for (j in (i + 1):dim(slice_Roman_PA)[1]) {
    newline <- data.frame(
      "Var1" = slice_Roman_PA$SiteCode[i],
      "Var2" = slice_Roman_PA$SiteCode[j],
      "CO_Roman" = (sum(
        slice_Roman_PA[i, 2:6] == slice_Roman_PA[j, 2:6]
      )) / 5
    )
...
# save dataset
write.csv(cooc_all, here("data", "df_CO_melted.csv"), row.names = FALSE)
