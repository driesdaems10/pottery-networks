...
# read cost matrix
costM <- as.matrix(read.csv(here("data", "cost_matrix.csv"), row.names = 1))

# read MIM matrix
MIM_Hellensitic <- as.matrix(read.csv(here("data", "MIM_Sites_HELLENISTIC.csv"),
    row.names = 1
))
MIM_Roman <- as.matrix(read.csv(here("data", "MIM_Sites_ROMAN.csv"),
    row.names = 1
))
...
# save the dataset
write.csv(df_all, here("data", "df_MI&Cost_melted.csv"), row.names = FALSE)
