...
# read data
data_catalogue <- read.csv(here("data/ICRATES", "ICRATES_CATALOGUE.csv"),
  header = TRUE, sep = ","
)
data_form <- read.csv(here("data/ICRATES", "ICRATES_STANDARD_FORM.csv"),
  header = TRUE, sep = ","
)

# check missing date
sum(is.na(data_form$Standard_Typo_chronological_Lower_Date)) / dim(data_form)[1]
# 20% of standard forms has no date info

# merge data
merged_data <- merge(data_catalogue, data_form,
  by = c("Standard_Form_ID", "Fabric"), all = TRUE
) # some mismatches in Fabric of data_form and data_catalogue
# retain only the ones with date info
merged_data_dates <- subset(
  merged_data,
  !(is.na(
    merged_data$Standard_Typo_chronological_Lower_Date
  ) | is.na(
    merged_data$Standard_Typo_chronological_Upper_Date
  )
  )
)
...
merged_data_dates <- merged_data_dates[
  ,
  c(colnames(merged_data_dates) %in% vars)
]
...
data_table <- as.data.frame(table(
  merged_data_dates$Standard_Form_ID
), stingsAsFactors = FALSE)
colnames(data_table)[1] <- "Standard_Form_ID"
final_table <- unique(
  merge(merged_data_dates, data_table, by = "Standard_Form_ID", all = TRUE)
)
...
step <- 50
# number of slices
nsl <- (max(final_table$Standard_Typo_chronological_Upper_Date) -
  min(final_table$Standard_Typo_chronological_Lower_Date)) / step
# breaks between slices
brk <- seq(
  min(final_table$Standard_Typo_chronological_Lower_Date),
  max(final_table$Standard_Typo_chronological_Upper_Date) + 1, step
)
brk
# cut to ranges
Ranges <- cut(final_table$Standard_Typo_chronological_Lower_Date,
...
data_intervals <- data.frame(
  "nrow" = 1:length(levels(Ranges)),
  "Year_Interval" = levels(Ranges)
)
for (i in 1:dim(final_table)[1]) {
  ts <- seq(
    final_table$Standard_Typo_chronological_Lower_Date[i],
    final_table$Standard_Typo_chronological_Upper_Date[i], 1
  )
  freqts <- as.data.frame(table(cut(ts,
    breaks = brk,
    include.lowest = TRUE,
    right = TRUE
  )), stringsAsFactors = F)
...
# order data
data_intervals <- data_intervals[order(data_intervals$nrow,
  decreasing = FALSE
), ]
# transpose data
data_intervals_estimated <- t(data_intervals)
# bring in relevant format before merging
data_intervals_estimated <- cbind(
  rownames(data_intervals_estimated),
  data_intervals_estimated
)
...
merged_data_dates <- subset(
  merged_data,
  !(is.na(merged_data$Standard_Typo_chronological_Lower_Date) | is.na(merged_data$Standard_Typo_chronological_Upper_Date))
)
vars <- c(
  "Location_ID",
  "Location_specific",
...
data_table <- as.data.frame(
  table(
    merged_data_dates$Standard_Form_ID,
    merged_data_dates$Location_ID
  ),
  stingsAsFactors = FALSE
)
colnames(data_table)[1:2] <- c("Standard_Form_ID", "Location_ID")
# include the fabric
data_fabric <- unique(cbind.data.frame(
  merged_data_dates$Standard_Form_ID,
  merged_data_dates$Fabric_ID
))
...
data_dates <- unique(merged_data_dates[
  ,
  c(colnames(merged_data_dates) %in% vars)
])
...
df_agg_fabric_loc <- aggregate(. ~ Fabric_ID + Location_ID,
  data = df_fabric_loc,
  FUN = sum
)
dim(df_agg_fabric_loc)[1] == sum(df_agg_fabric_loc$Freq == round(rowSums(df_agg_fabric_loc[, 4:23]))) # check result

# remove data from environment, keep only "df_agg_fabric_loc"
rm(df_form_loc, df_fabric_loc)

# Hellenistic slice (-150, -50]
df_slice_1 <- as.data.frame.matrix(
  xtabs(df_agg_fabric_loc[, 7] ~ Location_ID + Fabric_ID,
    data = df_agg_fabric_loc
  )
)
df_slice_2 <- as.data.frame.matrix(
  xtabs(df_agg_fabric_loc[, 8] ~ Location_ID + Fabric_ID,
    data = df_agg_fabric_loc
  )
)
df_slice_HELLENISTIC <- df_slice_1 + df_slice_2
write.csv(df_slice_HELLENISTIC, here("data", "slice_HELLENISTIC.csv"), row.names = TRUE)

# Roman slice (-50, 50]
df_slice_1 <- as.data.frame.matrix(
  xtabs(df_agg_fabric_loc[, 9] ~ Location_ID + Fabric_ID,
    data = df_agg_fabric_loc
  )
)
df_slice_2 <- as.data.frame.matrix(
  xtabs(df_agg_fabric_loc[, 10] ~ Location_ID + Fabric_ID,
    data = df_agg_fabric_loc
  )
)
df_slice_ROMAN <- df_slice_1 + df_slice_2
write.csv(df_slice_ROMAN, here("data", "slice_ROMAN.csv"), row.names = TRUE)
